...
import { freeStudyMaterials } from '../types/categories';
...
const verifyInput = (req: Request, res: Response, next: NextFunction) => {
    const linkInput = freeStudyMaterials.safeParse(req.body)
...
router.post("/link", verifyUser, verifyInput, async (req, res) => {
    ...
    try {
        const Link = await db.freeStudyMaterials.create({
            data: {
                title, desc: description, categories, link,
                userId
            }
        })
        if (!Link) {
...
router.delete("/link", verifyUser, async (req, res) => {
    ...
    try {
        const link = await db.freeStudyMaterials.delete({
            where: {
                id,userId
            }
        })
...
router.put("/link", verifyInput, verifyUser, async (req, res) => {
    ...
    const categories = req.body.categories
    try {
        const Link = await db.freeStudyMaterials.update({
            where: {
                userId,
                id
            }, data: {
